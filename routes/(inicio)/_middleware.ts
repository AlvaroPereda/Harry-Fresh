...
const generateCoockie = (user: string, token: string):Cookie => {
    ...
    const cookie: Cookie = { name: "user", value: encodeURIComponent(JSON.stringify(userObject)) }
    return cookie
}
...
const comprobarToken = async(user: string):Promise<boolean> => {
    ...
    const result = await HarryCollection.findOne({_id: new ObjectId(userObject.token)})
...
export const handler:Handler = async(req:Request, ctx:FreshContext) => {
    try {
        ...
        if(cookie.user) if(await comprobarToken(decodeURIComponent(cookie.user))) return await ctx.next()
        ...
        if(!email) {
            const result = await HarryCollection.findOne({name:user, password})
            if(!result) return devolver()
            return goHome(user, result._id.toString())
        } else {
            const result = await HarryCollection.findOne({email})
            if(result) return devolver()
            const { insertedId } = await HarryCollection.insertOne({name:user, email, password})        
